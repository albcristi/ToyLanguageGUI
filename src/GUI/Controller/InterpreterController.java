...
public class InterpreterController {
    ...
    ///TODO: DATA RELATED TO PROCEDURE TABLE
    @FXML
    private TableView<Map.Entry<String,String>> procedureTableView;

    @FXML
    private TableColumn<Map.Entry<String,String>, String> procedureColumn;

    @FXML
    private TableColumn<Map.Entry<String,String>, String> bodyColumn;
    ...
    void oneStepButtonClicked(MouseEvent event) {
     ...
     public void initialize(){
         ...
         procedureColumn.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getKey()+""));
         bodyColumn.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getValue()+""));
     }
    ...
    private void populateProcedureTable(){
        ProcedureTableInterface proc =controller.getProcTable();
        Map<String,String> proc_tab = new HashMap<>();

        for(String key: proc.keySet())
            proc_tab.put(key+proc.get(key).getKey().toString(),proc.get(key).getValue().toString());

        Map<String,String> pro = new HashMap<>();
        for(Map.Entry<String,String> entry: proc_tab.entrySet())
            pro.put(entry.getKey(),entry.getValue());

        List<Map.Entry<String,String>> fin = new ArrayList<>(pro.entrySet());
        this.procedureTableView.setItems(FXCollections.observableList(fin));
